...
# Faz o Uploud and Scan sem aguardar
try {
    # Caso exista um App ID, segue com os procedimentos
    if ($appID) {
        Clear-Host
        ...
        $uploudLog = VeracodeAPI.exe -vid $veracodeID -vkey $veracodeAPIkey -action uploadfile -appid "$appID" -filepath "$caminhoArquivo"
        Write-Host "Procedimento concluido"
    } else {
        Write-Host "Não foi encontrado um App ID para o $veracodeAppName"
    }
...
try {
    ...
    if ($hardcount -le 300) {
        ...
        # Exibe resultados do Uploud feito
        Clear-Host
        Write-Host "Log do Uploud feito:"
        $uploudLog
        Read-Host "Aperte enter para verificar o resultado do outro scan"
